...
<?php
    // to connect with the email 
    $my_email = "hassan.abuhadeer@triosstudent.com";
        ...
        if(substr_count($_REQUEST['email'],"@") != 1 || stristr($_REQUEST['email']," ")){
            $errors[] = "Email address is invalid";
                ...
                if(empty($exploded_email[0]) || strlen($exploded_email[0]) > 64 || empty($exploded_email[1])){
                    ...
                    if(substr_count($exploded_email[1],".") == 0){$errors[] = "Email address is invalid";
                    }
                    else{
...
<html>
    ...
    <body background ="#fff" text="#000">
        <div>

            <b>Thank you <?php if(isset($_REQUEST['name'])){print stripslashes($_REQUEST['name']);} ?></b>

            <br>Your message has been sent to Crypto Canuck!
